...
| 用户登录      | POST                            | /user/login     | 账号密码登录         | `username`, `password`                   | Response(),状态码400或者201                |
| 获取用户信息    | GET | /user/info      | 获取当前用户详情       | `token`（请求头）                             | `username`,`password`,`phone`,`email` |
| 更新用户信息    | PUT                             | /user/info      | 修改用户资料（昵称、签名等） | `nickname`, `signature`, `avatar_url`    | `success`（布尔值）                        |
| 收藏商家 / 菜品 | POST                            | /user/favorite  | 添加收藏记录         | `merchant_id`/`dish_id`, `type`（商家 / 菜品） | `favorite_id`                         |
| 获取收藏列表    | GET                             | /user/favorite  | 查询用户收藏的商家 / 菜品 | `token`, `type`（商家 / 菜品）                 | `list`（包含`merchant_id`/`dish_id`）     |
